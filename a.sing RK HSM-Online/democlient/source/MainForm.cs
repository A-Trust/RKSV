...
namespace DemoClient
{
    public partial class MainForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string[] parts = SplitJson(responseData);
            string sig = parts[1];
        ...
        private static string[] SplitJson(string responseData)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
                parts[i] = JsonStringDecode(parts[i]);
            return parts;
        }

        private static string JsonStringDecode(string input)
        {
            string temp = input;
            temp = temp.Replace("\\/","/");
            return temp; 
        ...
        private void button7_Click_1(object sender, EventArgs e)
        {
            ...
            string[] res = responseData.Split(new char[] { ':', ',', '}', '{', '"', ' ' }, StringSplitOptions.RemoveEmptyEntries);
            if (res.Length < 4)
            {
                tbResult.Text += "\r\nInvalid response";
                return;
            ...
            tbsessionid.Text = res[1];
            tbsessionkey.Text = res[3];
        }
        ...
        private void CheckInitData()
        {
            ...
            if(certserialhex.Length <= 0)
            {
                ...
                string[] parts = SplitJson(responseData);
                certserialhex = parts[parts.Length-3];
                sigcert = parts[1];
            }
        }
